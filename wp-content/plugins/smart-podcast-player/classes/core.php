...
class SPP_Core {
	...
	public function shortcode_smart_podcast_player( $atts = array() ) {
		...
		// Enqueue the Javascript file, unless this is a
		// Thrive Content Builder page (HS 3831) or the user has requested HTML assets
		$thrive_content_builder = false;
		$tve = filter_input( INPUT_GET, 'tve' );
		if ( $tve == 'true' ) {
			include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
	...
	public function shortcode_smart_track_player_latest( $atts = array() ) {
		...
		$transient_name = self::spp_transient_name( array(
				'name' => 'tracks from feed url',
				'url' => $atts['url'],
				'episode_limit' => 1 ) );
		...
		if( ( ( false === $data ) || !isset( $data['tracks'] ) ) || $no_cache == 'true' ) {
			...
			if ( is_array( $data['tracks'] ) || !empty ( $data['tracks'] ) ) {
					...
					if ( $val > 60 || $val < 5 || !is_numeric( $val ) )
						$val = 15;
					set_transient( $transient_name, $data, $val * MINUTE_IN_SECONDS);
			}
			else {
	...
	public function add_dynamic_css() {
		...
		foreach( $this->color_arrays as $color_array ) {
			...
			if( file_exists( $filename ) ) {
				...
				if( is_writable( SPP_ASSETS_PATH . 'css/' ) ) {
	...
	public function spp_transient_name( $name_args ) {
		...
		if( $name_args['purpose'] === 'tracks from feed url' ) {
			...
			$transient_name = 'spp_cachea_' . md5(
				self::VERSION . $name_args['url'] . (string) $name_args['episode_limit']);
